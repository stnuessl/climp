...
static int handle_args(struct climpd_control *__restrict cc,
                       const char **argv, 
                       int argc, 
                       int fd_out, 
                       int fd_err)
{
    struct vector list_args, add_args, remove_args;
    char *play_arg, *shuffle_arg, *repeat_arg, *volume_arg;
    bool pause, quit, previous, next;
